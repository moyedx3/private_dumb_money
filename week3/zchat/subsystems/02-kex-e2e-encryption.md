...
> ⚠️ **2026-05-16 정정**: 워크스루 step 5 의 "Alice 의 주소를 정말 통제하는 사람만이 그 주소에 묶인 pubkey signature 를 만들 수 있음" 표현이 부정확. **KEX 의 ECDSA 서명은 *A_pub 의 priv 소유* 만 증명하고, senderAddress 진위는 증명하지 못함** (Mallory 가 임의의 `senderAddress` 를 *자기* A_pub 와 묶어 서명 가능 — 검증 통과). 진짜 sender authentication 은 **out-of-band contact 등록 (TOFU)** 에 의존. ZMSG_PROTOCOL_SPEC.md Known Gaps 첫 항목 "Plain-ZMSG sender authentication 부재" 가 정직한 인정. [`../corrections-log.md` §G.2](../corrections-log.md) 참조.
...
여기 `verify`(line 436)는 `Signature.getInstance("SHA256withECDSA")`로 검증. ⚠️ **정정 2026-05-16**: 이 단계는 *A_pub 의 priv 소유* 만 증명할 뿐 — *senderAddress 의 진위*는 증명하지 못함. Mallory 가 임의의 `senderAddress` 를 *자기* `M_pub` 와 묶어 ECDSA(M_priv, senderAddress || M_pub) 로 서명하면 검증은 통과 (서명·pubkey 가 self-consistent 하므로). 즉 KEX 의 ECDSA 서명은 *내적 무결성*만 보장하고, *외부 trust anchor* (Bob 이 Alice 주소를 out-of-band 로 받아 contact 등록함, TOFU) 가 진짜 sender authentication 의 자물쇠.

> **(정정 2026-05-16)** 이전 표현 "Zcash transaction 의 송신을 통해 implicit 하게 증명" 은 **부정확**. Zcash 는 sender 익명 — Mallory 가 자기 wallet 의 노트를 spend 해서 memo 에 "I am Alice" 로 claim 가능. Zcash transaction 의 송신은 *spend authorization 가진 누군가* 가 보냈음을 증명할 뿐, *senderAddress 의 진위*를 증명하지 않음. 따라서 KEX 의 진짜 보안 모델은 "(a) ECDSA 서명으로 A_pub priv 소유 증명 + (b) Bob 이 *별도 채널로* Alice 주소를 받아 contact 등록 (TOFU, out-of-band)" 두 사실의 결합. (b) 가 없으면 (a) 만으로는 sender authentication 안 됨. ZMSG_PROTOCOL_SPEC.md Known Gaps 첫 줄 "Plain-ZMSG sender authentication 부재" 가 정직한 admission. [`../corrections-log.md` §G.2](../corrections-log.md)
