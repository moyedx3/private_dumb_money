...
- `VITE_DROP_PCCS_URL=` is optional. If unset, the bundled production QVL verifier uses Phala's default PCCS.
- `VITE_DROP_QVL_MODULE_URL=` is optional and only needed to override the bundled production QVL verifier. Local route-mocked QA may still install `window.dropQuoteVerifier`.

The app bundles `@phala/dcap-qvl` and uses it by default for live DCAP quote verification. The verifier fetches collateral, verifies the quote, accepts only `UpToDate` TCB status, returns TDX `RTMR3` as `codeMeasurement`, and returns quote `reportData` for the existing `report_data = sha256(pubkey)` check. To override it, provide a high-level verifier module via `VITE_DROP_QVL_MODULE_URL` or install `window.dropQuoteVerifier` before the app runs. Override modules must expose `verifyQuote(quoteHex)` or `verify(quoteHex)` and return `{ ok, codeMeasurement, reportData }` or the documented field aliases used by the tests.
...
```bash
VITE_DROP_INDEXER_URL="$LIVE_INDEXER_URL" VITE_DROP_EXPECTED_MEASUREMENT_HEX="$EXPECTED_MEASUREMENT" npm run qa:http-smoke
```

Live smoke calls `/health`, `/catalog`, and `/attest`, validates the I6 response, and verifies the quote with `scripts/production-qvl-verifier.mjs` unless `VITE_DROP_QVL_MODULE_URL` overrides it. Credential-bearing, query-bearing, or fragment-bearing indexer URLs are rejected and redacted from stdout and evidence.
