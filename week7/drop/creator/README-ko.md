...
서버를 믿고 `K_drop`을 주는 앱이 아니다. 브라우저가 먼저 quote를 검증해서 “이 공개키는 우리가 핀한 enclave 코드가 만든 키”라고 확인한 뒤에만, 그 공개키로 비밀을 sealed box에 넣는다. 검증 실패면 provisioning은 멈춘다.
...
핵심은 ②다. `quote` 검증과 공개키 바인딩이 통과해야만 ③으로 간다. 이걸 느슨하게 만들면 서버 운영자가 자기 공개키를 넣어 `K_drop`을 읽을 수 있으므로 제품 보장이 깨진다.
