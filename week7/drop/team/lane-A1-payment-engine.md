...
**Task 8 — ✅ 완료: 라이브 스모크 바이너리 (`src/bin/scan-live.rs`).** `A1_UFVK` 또는 `<creator-ufvk> [start] [end]`를 받아 real `GrpcClient` + 데모 드롭 하나짜리 in-memory Catalog + `bucket.put key=... len=...`만 찍는 로깅 Bucket로 `scan_once` 한 번. 실제 결제 블록 `3387683`에서 `bucket.put ... len=80`과 dispatch 1건을 확인했다.

**Task 9 — 🟡 경계 구현/TEE sealing 대기: encrypted cursor/replay state (`src/state.rs`).** `ScanState` + `MemoryScanState` + `EncryptedFileScanState` + `StateCipher` 경계를 추가했다. 개발용 구현은 dryoc secretbox(`SecretboxStateCipher`)로 파일에 암호문만 남긴다. `scan_once_with_state`는 이미 dispatch된 txid를 full tx fetch 전에 skip하고, dispatch 생성 txid와 `last_scanned_height`를 state에 반영한다. 운영에서는 `A1_STATE_KEY_HEX`가 아니라 enclave sealing key 기반 `StateCipher`로 교체해야 한다.

**Task 10 — 🟡 service vector 구현/HTTP adapter 대기: API endpoint vectors (`src/api.rs`).** `POST /api/creators/{creator_id}/drops`와 `GET /api/buyers/dispatch/{bucket_key}` contract를 추가했다. 현재는 HTTP 서버가 아니라 `ApiVectors` service layer이며, `Catalog`+`Bucket`을 동시에 구현한다. 즉 creator 등록 → scanner catalog lookup → engine dispatch put → buyer bucket lookup 흐름이 단위 테스트로 고정됐다. 운영에서는 attested enclave ingress와 encrypted DB-backed store로 교체해야 한다.


## 4.1 현재 검수 결과 / 단계 표기 (2026-06-24)

상태 기준: ✅ 완료 · 🟡 부분 완료 · ⬜ 미시작. 현재 구현은 가이드의 큰 방향과 맞고, 이제 **encrypted cursor/replay state 경계(Task 9)** 까지 구현됐다. 아직 실제 TEE sealing key 연동과 운영용 long-running scanner가 남아 있다.
...
| Task 8 — live smoke binary | ✅ | 구현 및 live 검증 완료. `A1_UFVK`/인자, `A1_SCAN_START/A1_SCAN_END`, demo drop 설정으로 실제 lightwalletd에서 `scan_once`를 실행했고 height `3387683`에서 `bucket.put ... len=80`/`dispatch ...` 1건 확인. | `src/bin/scan-live.rs`. |
| Task 9 — encrypted cursor/replay state | 🟡 | state 저장 경계 구현 및 단위 테스트 완료. 파일에는 secretbox 암호문만 저장되고, state-aware scan이 이미 본 txid를 skip한다. 운영용 TEE sealing key 구현은 남음. | `src/state.rs`, `src/scan_loop.rs`, `src/bin/scan-live.rs`. |
| Task 10 — API service vectors | 🟡 | creator drop 등록 vector와 buyer dispatch 조회 vector 구현 및 단위 테스트 완료. HTTP adapter/attestation ingress/DB-backed store는 남음. | `src/api.rs`, `a1-api-vectors.md`. |

현재 검수 결론: **Task 0~10 service vector까지 완료됐고, 전체 완성률은 약 87%**다. 다음 연결 작업은 HTTP/enclave adapter, enclave sealing key adapter, 운영용 polling service loop다.
