...
3. **[TEE narrative 의 두 전제조건](#33--신뢰가능한-감사-결과의-두-가지-전제조건) (§3.3, 운영 필수)** — TEE 가 의미 있으려면:
   - **(a) Viewing key 는 RA-TLS 채널로만** (운영자도 평문 못 봄) — D10. *현재 passthrough 배포라 client 가 enclave 를 직접 검증 + measurement 핀(TOFU). 완성(단계 2)은 demo-architecture-limitations.md §4.*
...
→ 우리 narrative 가 "non-interaction with sanctioned (downstream outgoing)" 좁은 claim 으로 설정된 이유 (decisions D1).
...
| 항목 | 우선순위 | 후속 ID |
|---|---|---|
| 2.1 receiver canonicalization 부재 (★ encoding mismatch 의 진짜 원인) | 🔴 높음 | D14 |
...
`runRealScan` 이 OVK 로 outgoing 만 추출. incoming 은 `scan-incoming` 디버그 도구로만 분리. 본체 통합 가능 (~30줄), 단 §1.2 의 본질적 약점(shielded sender 식별 불가)으로 narrative 약함.

### 2.7 RA-TLS quote verifier 가 Phala API 의존

`submit-ufvk` 의 RA-TLS verify 가 `cloud-api.phala.com` 호출. local 검증 옵션 없음. 현재 passthrough
배포에서 **가동 중**(quote DCAP 를 이 API 에 위임) → Phala verifier 가용성/신뢰가 신뢰 컴포넌트로 남음.
...
- 깨지는 케이스:
  ...
  - client 가 quote 검증 없이 `--no-verify` 로만 보내면 표준 TLS 검증까지 꺼져(`rejectUnauthorized:false`) "임의의 enclave 인 척하는 server" 가 가로채도 모름 (능동 MITM 노출). 이 모드는 **대안(gateway 종단) 배포** 에서만 — 현재 데모는 passthrough + client 검증 (아래 "배포 config").
...
**배포 config (현재 = passthrough).** 현재 데모는 gateway TLS passthrough + `SCANNER_TRANSPORT=ratls` + submit-ufvk `--no-verify` 제거 → client 가 enclave 를 직접 검증(quote + pubkey 바인딩 + measurement 핀 + 채널 pin), **gateway 는 신뢰주체 아님**. measurement 값은 아직 TOFU 라 "정직한 코드"는 미완(단계 2). 대안(약한 모드 = gateway 종단 + `http` + `--no-verify`)과 단계별 비교·완성 경로는 → **[demo-architecture-limitations.md](./demo-architecture-limitations.md)**.

→ Phala 배포 시 두 조건 모두 점검 필수 (deploy-phala.md §4 헬스체크 + §6 quote 외부 검증 + 거래소 측 policy allowlist 검토) + 위 배포 config 차이 확인.
...
**현재 운영 함의**:
- shielded 송금에 대한 OFAC 매칭은 현실에서 거의 발생 안 함. §1.1·1.2 의 shielded 한계가 실 운영 영향 작음.
...
원안 (초기 mock-JSON ZK 안): 사용자가 outgoing record 목록을 직접 JSON 제출 + ZK 로 non-intersection 증명. **completeness 못 푼다** — 사용자가 sanctioned 든 record 를 witness 에서 빼면 ZK 통과. attested scanner 로 전환 (D1·D2).
...
- 후속 작업(F1~F4)은 위 §2 표 참고
- `ONBOARDING.md` (루트) — 배포·테스트 진입점
